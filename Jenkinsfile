pipeline {  
    ...
    environment {  
        ...
        DEPLOY_SERVER = '43.205.196.41'  
    }  
  
    stages {  
        stage('Checkout') {  
            steps {  
                git branch: 'main', url: 'https://github.com/Jignasa-Dabhi/website-docker-demo.git'  
            }  
        }  
        ...
        stage('Login to ECR') {
    steps {
        sh(script: "aws ecr get-login-password --region ap-south-1 | docker login --username AWS --password-stdin 314076239426.dkr.ecr.ap-south-1.amazonaws.com")
    }
}
